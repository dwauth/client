...
test('register another user', function (t) {
  client.register({email: 'test@dwebs.io', password: 'verysecret'}, function (err, res, body) {
    t.error(err, 'no error')
    t.pass('registers')
    t.same(body.email, 'test@dwebs.io', 'returns email back with request')
...
test('secure request with new info', function (t) {
  var clientToo = DwidClient({
    server: address,
    routes: {
      register: '/fakeRegister'
    }
  })
  clientToo.register({email: 'test@dwebs.io', password: 'verysecret'}, function (err, res, body) {
    t.ok(err, 'gets error')
    t.end()
  })
})
